...
class Exercises {
    ...
    public function removeExistSubmit($userId, $exerId) {
        $query = 'DELETE FROM submit_exercise WHERE id = ? AND exerciseId = ?';
        $stmt = $this->conn->prepare($query);
    ...
    public function submit($id, $userId, $url) {
        ...
        $query = 'INSERT INTO submit_exercise SET exerciseId = ?, url = ?, id = ?, created = now()';
        $stmt = $this->conn->prepare($query);
    ...
    public function getSubmit($userId) {
        $query = 'SELECT exerciseId, url FROM submit_exercise WHERE id = ?';
        $stmt = $this->conn->prepare($query);
